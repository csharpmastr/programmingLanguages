...
async function contrastChecker() {
    let textColorValue = textColor.value;
    let bgColorValue = bgColor.value;

    // Perform asynchronous operation
    const [textColorRGBArray, bgColorRGBArray] = await Promise.all([
        hexToRGB(textColorValue),
        hexToRGB(bgColorValue)
    ]);

    // Calculate contrast ratio asynchronously
    const contrast = await calculateContrastRatio(textColorRGBArray, bgColorRGBArray);
    ...
    // Alert after UI updates
    setTimeout(window.alert(contrast.toFixed(2)), 1000);
}
